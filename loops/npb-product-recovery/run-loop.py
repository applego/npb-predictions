...
class Runner:
    def __init__(self, spec_path: Path) -> None:
        ...
        project_root = self.spec.get("workspace", {}).get("project_root")
        self.project_root = relative_to_base(project_root, self.base_dir) if project_root else self.base_dir
    ...
    def gather_context(self) -> str:
        ...
        for index, source in enumerate(goal.get("context_sources", []), start=1):
            ...
            if "file" in source:
                ...
                result = run_argv(argv, cwd=self.project_root, timeout_sec=int(source.get("timeout_sec", 60)))
                chunks.append(
    ...
    def run_programmatic(self, criterion: dict[str, Any]) -> dict[str, Any]:
        ...
        result = run_argv(argv, cwd=self.project_root, timeout_sec=int(criterion.get("timeout_sec", 300)))
